...
- A 404 `resource_not_disclosed` from the observability routes now degrades the platform panes as `record-not-disclosed` ("the record is not on this platform, or not disclosed to this account; retrying will not change that") instead of a retryable `platform-unavailable`; the evidence pane gets the matching unavailable reason.
