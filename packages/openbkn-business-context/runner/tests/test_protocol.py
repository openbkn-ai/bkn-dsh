...
class DecodeRequestTests(unittest.TestCase):
    ...
    def test_accepts_only_a_host_bound_interaction_id_for_provenance_reads(self) -> None:
        ...
        self.assertEqual(request.context, {"interaction_id": "int-123"})

    def test_rejects_an_arbitrary_route_in_provenance_context(self) -> None:
        with self.assertRaisesRegex(ProtocolError, "context"):
            decode_request(
                {
                    "version": 1,
                    "operation": "get_interaction_operations",
                    "context": {"interaction_id": "int-123", "path": "/anything"},
                }
            )
