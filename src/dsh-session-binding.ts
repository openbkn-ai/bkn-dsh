...
export interface DshSessionBindingLog extends DshSessionLog {
  append(type: typeof BUSINESS_NETWORK_BOUND_EVENT, data: SessionEventMap[typeof BUSINESS_NETWORK_BOUND_EVENT]): unknown
}
...
export function bindDshSessionBusinessNetwork(
  ...
  if (result.kind === 'bound') session.append(result.event.type, result.event.data)
  return result
}
