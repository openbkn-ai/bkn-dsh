...
补丁仅提供插件所需的三项能力：Streamable HTTP MCP 的凭证引用请求头、外部插件的已发布 Typert 协议识别，以及可忽略的插件非界面会话记录。它不会读取或写入 OpenBKN Token。
...
按 DSH 自身的构建说明重新构建该源码工作树，然后仍通过 DSH 原生命令安装 bkn-dsh：
