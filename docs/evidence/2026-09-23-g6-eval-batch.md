...
> 判分记录：`docs/eval/answers-2026-09-23.json`；机器汇总：`docs/evidence/2026-09-23-g6-eval-results.md`（**正例 6/6 · 负例 2/4 · 总 8/10**）。
...
| missing-object | PASS | 明确「物料不存在」并逐表 0 行核验；判分口径：②「不给出任何数值」按「不编造业务数值」解释，0 命中表是「无数据」的证明而非业务数值（轮 23，如实记录解释） |
...
| platform-unreachable | PASS（附批注） | 表现为 **DSH 客户端级整轮报错横幅**（"This turn failed … fetch failed"），模型未介入；判据①连接失败提示明确✓、②会话可用✓（恢复后下一轮正确作答 375/816）。批注：期望中「模型在轮内优雅报告」未发生——重连探测失败即终止轮次；不违反任何禁止项（无编造、无挂死） |
